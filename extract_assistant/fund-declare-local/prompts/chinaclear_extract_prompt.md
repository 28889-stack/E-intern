...
18. 为避免 JSON 截断，每个 batch 最多输出 12 条业务记录；如果明显超过 12 条，优先输出当前 batch 中定位最清晰的 12 条，不要额外输出猜测性的“可能仍有未输出记录”提示。
19. 所有 `message` 不超过 80 个中文字符；所有 `source_evidence.raw_text` 不超过 120 个中文字符；不要把 OCR 原文整段复制进 JSON。
