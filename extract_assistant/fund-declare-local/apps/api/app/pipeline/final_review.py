...
COMPLETE_REVIEW_COLUMNS = FINAL_REVIEW_COLUMNS
HOLDING_REVIEW_COLUMNS = [
    "账户类型",
    "证券账号",
    "证券代码",
    "证券名称",
...
CHECKLIST_REVIEW_COLUMNS = [
    "checklist条件",
    "状态",
    "说明",
]
PROBLEM_REVIEW_COLUMNS = [
    "问题ID",
    "问题对象类型",
    "问题类型",
    "严重程度",
...
def get_review_payload(case_id: str) -> dict:
    ...
    reviewed_path = _reviewed_final_result_path(case_id)
...
def save_review_payload(case_id: str, payload: dict) -> dict:
    ...
    review_data = {
        ...
        SHEET_PROBLEMS: server_review_data.get(SHEET_PROBLEMS, []),
    }
    ...
    reviewed = {
        ...
        "problem_events": final_result.get("problem_events", []),
        "pending_review_events": final_result.get("pending_review_events", []),
        "pending_review_holdings": final_result.get("pending_review_holdings", []),
        "merge_audit": final_result.get("merge_audit", []),
...
def reviewed_final_result_path(case_id: str) -> Path:
    return _reviewed_final_result_path(case_id)
...
def build_review_data_from_final_result(case_id: str, final_result: dict) -> dict:
    ...
    problem_rows = _sheet_rows(final_result, SHEET_PROBLEMS)
    ...
    return {
        ...
        SHEET_PROBLEMS: [_problem_review_row(row) for row in problem_rows],
    }
...
def _problem_review_row(row: dict) -> dict:
    return {column: row.get(column, "") for column in PROBLEM_REVIEW_COLUMNS}
...
def _account_type(value: Any) -> str:
    ...
    if market in {"SZ", "深A", "深圳", "深市"}:
        return "深A"
    if market in {"SH", "沪A", "上海", "沪市"}:
        return "沪A"
    return market
...
def _trace_from_final_result(
    ...
    return {
        ...
        "problem_event_count": len(final_result.get("problem_events", [])),
    }
...
def _reviewed_final_result_path(case_id: str) -> Path:
